...
def extract_predicted_label(coa_text):
    ...
    reasoning_end = coa_text.find("分类结果")
    if reasoning_end == -1:
        return None  # 无预测标签

    # 提取 </REASONING> 之后的文本
    text_after_reasoning = coa_text[reasoning_end + len("分类结果"):]
...
def main():
    # 加载数据
    with open("cache/pred.json", "r", encoding="utf-8") as f:
        data = json.load(f)
    ...
    with open("cache/eval_pred.json", "w", encoding="utf-8") as f:
